...
var cryptoNight = cnHashing['cryptonight'];
var randomX = cnHashing['randomx'];

var cn = multiHashing['cryptonight'];
...
var isKevacoin = config.coin === "kevacoin";
...
(function init(){
    if (!utils.isValidPoolAddress(config.poolServer.poolAddress)) {
        log('error', logSystem, 'Invalid pool address: ' + config.poolServer.poolAddress + '. Could not start pool');
        return;
    }
...
function Miner(id, login, pass, ip, startingDiff, fixedDiff, pushMessage){
    this.id = id;
    this.login = login;
    this.pass = pass;
    this.ip = ip;
...
Miner.prototype = {
    ...
    getTargetHex: function(){
        ...
        var buffArray = buff.toJSON().data;
        buffArray.reverse();
    ...
    getJob: function(){
        ...
        var majorVersion = parseInt(blob[0] + blob[1], 16);
        var job = {
            blob: blob,
            job_id: newJob.id,
            target: target,
        ...
        if (majorVersion == 10) {
            ...
            job.algo = 'rx/keva';
            job.seed_hash = currentBlockTemplate.seed_hash;
            job.next_seed_hash = currentBlockTemplate.next_seed_hash;
        }
        return job;
    },
...
function processShare(miner, job, blockTemplate, nonce, resultHash){
    ...
    if (shareTrustEnabled && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 && Math.random() > miner.trust.probability){
        ...
        var cn_variant;
        if (convertedBlob[0] == 10) {
            // Keva CN/R
            cn_variant = 13;
            hash = cryptoNight(convertedBlob, cn_variant, blockTemplate.height);
        } else if (convertedBlob[0] == 12) {
            // Keva RandomX
            var seed_hash = Buffer.from(job.seed_hash, "hex");
            hash = randomX(convertedBlob, seed_hash, 19);
        }
        shareType = 'valid';
    }
    ...
    var hashArray = hash.toJSON().data;
    hashArray.reverse();
    ...
    if (hashDiff.ge(blockTemplate.difficulty)){
        ...
        apiInterfaces.rpcDaemon('submitblock', [shareBuffer.toString('hex')], function(error, result){
            if (error){
                ...
                var hashValue = cryptoNightFast(convertedBlob || cnUtil.convert_blob(shareBuffer));
                var blockFastHash = reverseBuffer(hashValue).toString('hex');
                log('info', logSystem,
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage){
    ...
    switch(method){
        case 'login':
            ...
            miner = new Miner(minerId, params.login, params.pass, ip, portData.difficulty, portData.fixedDiff, pushMessage);
