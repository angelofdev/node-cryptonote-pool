...
function processShare(miner, job, blockTemplate, nonce, resultHash){
    ...
    if (shareTrustEnabled && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 && Math.random() > miner.trust.probability){
        ...
        //var cn_variant = (isMonero || isKevacoin) && convertedBlob[0] >= 7 ? convertedBlob[0] - 6 : 0;
        var cn_variant;
        if (convertedBlob[0] == 10) {
            ...
            var blockhash = Buffer.from(job.seed_hash);
            hash = randomX(convertedBlob, blockhash, 0);
        }
        hash = cryptoNight(convertedBlob, cn_variant, blockTemplate.height);
        shareType = 'valid';
    }
